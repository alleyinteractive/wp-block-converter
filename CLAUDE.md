...
- `Block` (`src/Block.php`) is a plain value object (block name/attributes/content) that renders via WordPress' `get_comment_delimited_block_content()`.
...
- Autoloading is standard Composer PSR-4 (no `composer-wordpress-autoloader`), so filenames match class/trait names exactly (e.g. `src/Block_Converter.php`, `src/Concerns/Listens_For_Attachments.php`). Class/trait names themselves keep the Alley convention of underscore-separated names (e.g. `Block_Converter`), and methods/properties stay snake_case.
